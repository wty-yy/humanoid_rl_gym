...
if __name__ == "__main__":
    ...
    with open(f"{LEGGED_GYM_ROOT_DIR}/deploy/deploy_mujoco/configs/{config_file}", "r") as f:
        ...
        gait_phase_cycle = config["gait_phase"]
    ...
    step_count = 0  # counts policy steps for gait phase
    ...
    mujoco_render_utils = MujocoRenderUtils(arrow_z_offset=0.6)
    ...
    with mujoco.viewer.launch_passive(m, d) as viewer:
        ...
        while viewer.is_running() and time.time() - start < simulation_duration:
            ...
            if counter % control_decimation == 0:
                ...
                # gait phase: sin/cos of 2*pi*phase
                phase = step_count * simulation_dt * control_decimation / gait_phase_cycle
                gait_sin = np.sin(2 * np.pi * phase)
                gait_cos = np.cos(2 * np.pi * phase)
                step_count += 1
                ...
                obs[9 + 3 * num_actions] = gait_sin
                obs[9 + 3 * num_actions + 1] = gait_cos
